import sys

import asyncio
import aio_pika
import pika
from dotenv import load_dotenv
import os

load_dotenv()
token = os.environ.get("api-token")

RABBITMQ_URL = os.getenv("RABBITMQ_URL")
RABBITMQ_PORT = int(os.getenv("RABBITMQ_PORT"))
RABBITMQ_USERNAME = os.getenv("RABBITMQ_USERNAME")
RABBITMQ_PASSWORD = os.getenv("RABBITMQ_PASSWORD")

MY_QUEUE_NAME = 'my_queue'


def publish_message_my_queue(message: str):
    """Publish a message to the queue

    This function will publish a message to the queue."""
    connection = pika.BlockingConnection(pika.ConnectionParameters(host=RABBITMQ_URL, port=RABBITMQ_PORT,
                                                                   credentials=pika.PlainCredentials(RABBITMQ_USERNAME,
                                                                                                     RABBITMQ_PASSWORD)))
    channel = connection.channel()
    channel.queue_declare(queue=MY_QUEUE_NAME)
    channel.basic_publish(exchange='', routing_key=MY_QUEUE_NAME, body=message)
    connection.close()


async def my_queue_callback(message: aio_pika.IncomingMessage):
    """Callback function for the queue

    This function will be called when a message is received from the queue."""
    async with message.process():
        body = message.body
        properties = message.properties
        print(f" [x] Received {body}")


async def wait_for_close(channel):
    """Wait for the channel to be closed

    This function will wait for the channel to be closed."""
    while not channel.is_closed:
        await asyncio.sleep(1)


async def setup_rabbitmq(loop: asyncio.AbstractEventLoop):
    """Setup RabbitMQ connection and channel"""
    print(' [*] Setting up RabbitMQ connection and channel')
    queues: list[tuple[str, callable]] = [(MY_QUEUE_NAME, my_queue_callback)]
    connection = await aio_pika.connect_robust(host=RABBITMQ_URL, port=RABBITMQ_PORT,
                                                  login=RABBITMQ_USERNAME, password=RABBITMQ_PASSWORD, loop=loop)

    channel = await connection.channel()
    for queue in queues:
        rabbit_queue = await channel.declare_queue(queue[0])
        await rabbit_queue.consume(queue[1])
    # Wait for the channel to be closed
    loop.create_task(wait_for_close(channel))

    return channel


if __name__ == "__main__":
    """Main function to run the script

    This function will run the script and setup the RabbitMQ connection and channel."""
    try:
        print(' [*] Waiting for messages. To exit press CTRL+C')
        setup_rabbitmq()
    except KeyboardInterrupt:
        print('Interrupted')
        try:
            sys.exit(0)
        except SystemExit:
            os._exit(0)
